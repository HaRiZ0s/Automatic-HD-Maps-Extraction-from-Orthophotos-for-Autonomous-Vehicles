...
#    mineimage = plt.imread('nighthouseroad.jpg')
#    img = plt.imshow(mineimage)
#    plt.show()


    for i, ((image, calib, grid2d), (cls_map, vis_mask)) in enumerate(dataloader):
        #img = plt.imshow(np.transpose(image[0].numpy(),(1,2,0)))
        #plt.show()
        # Move tensors to GPU
        ...
        #pred_ms = model(image[0], calib[0], grid2d[0])
...
# Visualize score
    ...
    fig.clear()
#    new_im = Image.new('RGB', (2*cls_idx.size[0],2*cls_idx.size[1]), (250,250,250))

#   new_im.paste(cls_idx, (0,0))
#   new_im.paste(scores, (cls_idx.size[0],0))
    ...
    #print("scores : ",scores)
    #scores_threshold = np.where(scores > 0.5, scores, 0)
...
def main():
    ...
    if args.pretrained_bem:
        print("loaded 2222pretrained model")
    ...
    #            and "C:\\Users\\giann\\miniconda3\\envs\\planb\\translating-images-into-maps-main\\experiments\\checkpoint-0008.pth.gz" in f
    #        ]
    #    )
...
#    print(model)
#    path=os.path.abspath("C:/Users/giann/miniconda3/envs/planb/translating-images-into-maps-main/experiments/dokimes3/checkpoint-0008.pth.gz")
...
#    model.to(device)
    ...
    checkpt_fn = sorted(
        [
            ...
            if os.path.isfile(os.path.join(model_dir, f)) and "C:\\Users\\giann\\miniconda3\\envs\\planb\\translating-images-into-maps-main\\experiments\\dokimes5\\checkpoint-0008.pth.gz" in f
        ]
    )
    #"C:\\Users\\giann\\miniconda3\\envs\\planb\\translating-images-into-maps-main\\experiments\\dokimes5\\checkpoint-0008.pth.gz"
    if len(checkpt_fn) != 0:
    ...
    for epoch in range(epoch_ckpt, args.epochs + 1):
        ...
        if epoch % args.val_interval == 0:
            ...
            validate(args, val_loader, model, epoch_ckpt)
